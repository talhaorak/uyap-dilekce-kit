...
export function renderPetitionMarkdown(petition: Petition): string {
  return [
    ...
    field("BAŞVURAN", renderParty(petition.applicant)),
    petition.respondent ? field("KARŞI TARAF / İDARE", petition.respondent) : "",
    field("KONU", petition.subject),
    section("AÇIKLAMALAR", numbered(petition.explanations)),
    optionalSection("HUKUKİ SEBEPLER", petition.legalGrounds),
    optionalSection("DELİLLER", petition.evidence),
    section("SONUÇ VE İSTEM", numbered(petition.requests)),
    optionalSection("EKLER", petition.attachments),
    petition.date ? field("TARİH", petition.date) : "",
    "Başvuran",
...
export function renderPetitionText(petition: Petition): string {
  return [
    petition.court,
    "",
    textField("BAŞVURAN", renderParty(petition.applicant)),
    petition.respondent ? textField("KARŞI TARAF / İDARE", petition.respondent) : "",
    textField("KONU", petition.subject),
    "",
    textSection("AÇIKLAMALAR", numbered(petition.explanations)),
    optionalTextSection("HUKUKİ SEBEPLER", petition.legalGrounds),
    optionalTextSection("DELİLLER", petition.evidence),
    textSection("SONUÇ VE İSTEM", numbered(petition.requests)),
    optionalTextSection("EKLER", petition.attachments),
    petition.date ? textField("TARİH", petition.date) : "",
    "",
    "Başvuran",
    petition.applicant.fullName,
  ]
